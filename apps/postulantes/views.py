...
from .forms import RegistroPostulanteForm
...
def registrar(request):
    if request.method == 'POST':
        form = RegistroPostulanteForm(request.POST, request.FILES)
        if form.is_valid():
            postulante = form.save(commit=False)
            postulante.save()
    
            # Generar el PDF
            pdf_content = generar_pdf(postulante)

            # Envía un mensaje de confirmación al correo del postulante
            subject = 'Confirmación de registro'
            message = 'Gracias por registrarte. Adjunto encontrarás tu confirmación de registro.'
            ...
            email.attach(f'confirmacion_registro_{postulante.dni}.pdf', pdf_content, 'application/pdf')
        ...
        form = RegistroPostulanteForm()
    return render(request, 'postulantes/registrar_postulante.html', {'form': form})
...
def listar_postulantes(request):
    ...
    return render(request, 'postulantes/listar_postulantes.html', {'postulantes': postulantes})
...
def postulante_print(self, pk=None):  
    ...
    elements.append(Paragraph("PROCESO DE ADMISIÓN IESTP ARIB - 2024", titulo2))
